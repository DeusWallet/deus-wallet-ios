...
class App {
    ...
    let evmRpcAvManager: EvmRpcAvManager
    ...
    init() throws {
        ...
        keychainStorage = KeychainStorage(service: "io.deus.wallet.dev")
        ...
        let evmRpcAvStorage = EvmRpcAvStorage(dbPool: dbPool)
        let evmRpcAvProvider = EvmRpcAvProvider()
        evmRpcAvManager = EvmRpcAvManager(evmRpcAvStorage: evmRpcAvStorage, evmRpcAvProvider: evmRpcAvProvider, logger: logger)

        let evmSyncSourceStorage = EvmSyncSourceStorage(dbPool: dbPool)
        evmSyncSourceManager = EvmSyncSourceManager(testNetManager: testNetManager, blockchainSettingsStorage: blockchainSettingsStorage, evmSyncSourceStorage: evmSyncSourceStorage, evmRpcAvManager: evmRpcAvManager)
        ...
        let walletClientInfo = WalletConnectClientInfo(
            projectId: AppConfig.walletConnectV2ProjectKey ?? "",
            relayHost: "relay.walletconnect.com",
            name: AppConfig.appName,
            description: "",
            url: AppConfig.appWebPageLink,
            icons: ["https://deuswallet.com/images/img/icons/D-logo.jpg"]
        )
