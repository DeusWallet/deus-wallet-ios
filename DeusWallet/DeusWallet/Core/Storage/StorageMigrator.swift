...
enum StorageMigrator {
    static func migrate(dbPool: DatabasePool) throws {
        ...
        migrator.registerMigration("Create EvmRpcAvRecord") { db in
            try db.create(table: EvmRpcAvRecord.databaseTableName) { t in
                t.column(EvmRpcAvRecord.Columns.chainId.name, .text).notNull()
                t.column(EvmRpcAvRecord.Columns.url.name, .text).notNull()
                t.column(EvmRpcAvRecord.Columns.height.name, .integer).notNull()
                t.column(EvmRpcAvRecord.Columns.latency.name, .integer).notNull()

                t.primaryKey([EvmRpcAvRecord.Columns.chainId.name, EvmRpcAvRecord.Columns.url.name], onConflict: .replace)
            }
        }
