...
class EvmSyncSourceManager {
    ...
    private let evmRpcAvManager: EvmRpcAvManager

    private let syncSourceRelay = PublishRelay<BlockchainType>()
    private let syncSourcesUpdatedRelay = PublishRelay<BlockchainType>()

    init(testNetManager: TestNetManager, blockchainSettingsStorage: BlockchainSettingsStorage, evmSyncSourceStorage: EvmSyncSourceStorage, evmRpcAvManager: EvmRpcAvManager) {
        ...
        self.evmRpcAvManager = evmRpcAvManager
    }
    ...
    func defaultSyncSources(blockchainType: BlockchainType) -> [EvmSyncSource] {
        ...
        case .ethereum:
            if testNetManager.testNetEnabled {
                ...
                var evmSyncSourceList:[EvmSyncSource] = []
                let evmRpcAvSyncSourceList = try? evmRpcAvManager.availableRpc(chainId: 1, transactionSource: defaultTransactionSource(blockchainType: blockchainType))
                            
                if evmRpcAvSyncSourceList != nil {
                    evmSyncSourceList += evmRpcAvSyncSourceList!
                }
                 
                return [
                    EvmSyncSource(
                        name: "Ankr",
                        rpcSource: .http(urls: [URL(string: "https://rpc.ankr.com/eth")!], auth: nil),
